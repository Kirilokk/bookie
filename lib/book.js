...
export async function getShelfBookIds() {
  try {
    const result = await pool.query(
      `SELECT 
        api_book_id
    ...
    console.error("Помилка під час вибірки id книг ", error);
  }
}
  ...
  try {
    const result = await pool.query(
      `SELECT 
        ...
        books.publish_date,
        authors.full_name AS author,
        publishers.full_name as publisher,
        COALESCE(
          ARRAY_AGG(book_isbns.value) FILTER (WHERE book_isbns.value IS NOT NULL),
          '{}'
        ) AS isbns
      FROM books
      JOIN authors ON authors.id = books.author_id
      LEFT JOIN publishers ON publishers.id = books.publisher_id
      LEFT JOIN book_isbns ON book_isbns.book_id = books.id
      WHERE books.id = $1
      GROUP BY 
        books.id,
        authors.id,
        publishers.id;`, [book_id]
    )
...
export async function insertBook(bookData) {
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO books (title, author_id, publisher_id, page_count, status, cover_color, image_url, api_book_id, publish_date)
    VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
    RETURNING *`, [bookData.title, authorId, publisherId, bookData.page_count, bookData.status, bookData.coverColor, bookData.image, bookData.api_book_id, bookData.publishedDate]);
...
export async function isBookInShelf(book_id) {
  const all_books = await getAllBooks()
  return all_books.some((b) => b.api_book_id === book_id)
}
